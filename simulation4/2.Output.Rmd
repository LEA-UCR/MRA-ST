...
scenario<-paste0("sim_res/chain",i,model,taper,type,nres,".Rdata")
print(scenario)
load(scenario)
print(unlist(labels))
print(total_time)
...
```{r plot3}
Nu <- chain$chain_result$chain_Nu[-(1:burn)]
Nu <- data.frame(Nu=Nu) %>% mutate(Index=1:n())
grafico_Nu <- ggplot(data = Nu,mapping = aes(x = Index,y = Nu))+
  geom_line()+
  theme_bw()
grafico_Nu
colMeans(Nu)
