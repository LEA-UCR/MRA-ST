...
function.to.gen.data <- function(nDEP,nres,i){
  ...
  type <- DEP[nDEP]
  phi <- 3
  nu <- 3/2  # roughness parameter for error
  sigma2 <- sigma2
  tau2 <- 1
  ...
  ### ############################ ###
  rExpMat_regional <- function(n,coords,type,phi,nu=nu){
    if(type=='Exponential'){
  ...
  ### ############################ ###
  beta0.cov <- rExpMat_regional(n,gridbase_regional,type = type,phi=phi,nu=nu)
