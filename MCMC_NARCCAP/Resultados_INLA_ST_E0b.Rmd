---
title: "Resultados y Diagnósticos-INLA-ST_E0B-MODELO_1"
output:
  html_document:
    df_print: paged
---
...
Calcula métricas y resultados del escenario 1 en Barboza et al, 2022. Carga de resultados:
```{r}
load('results/results_INLA_ST_E0b.RData')
```
...
write_csv(x = resumenObs_year,file = 'tabla_res_1.csv')
```
...
monson_sf_baseplot <- bind_rows(data_monson_join_sf_03,
  ...
  rename(Observed = temp, RCM = ts, Emulator = That, QRange = TR) %>%
  pivot_longer(names_to = 'Variable', cols = all_of(c('Observed', 'RCM', 'Emulator', 'QRange')), values_to = 'Value')
...
grafico_Temp_monson <- ggplot(data = monson_sf_baseplot_Temp)+geom_sf(aes(col = Value))+
  ...
  theme_bw()+
  scale_color_distiller(type = 'seq',palette = 'YlOrRd',direction = 1)+
  theme(axis.text.x = element_text(size = 5),legend.title = element_blank())
...
plot_grid(grafico_Temp_monson,grafico_Rango_monson,nrow = 2,ncol = 1,rel_heights = c(3,1))
```

Tabla de parámetros:

```{r}
tabla_param <- bind_rows(m.ex2$summary.fixed[c(1,3,5)],
m.ex2$summary.hyperpar[c(2,3),c(1,3,5)],
1/m.ex2$summary.hyperpar[1,c(1,3,5)])
