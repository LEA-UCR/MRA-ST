---
title: "Resultados y Diagnósticos-INLA-ST_E0C-MODELO_2"
output:
  html_document:
    df_print: paged
---
...
Calcula métricas y resultados del escenario 2 en Barboza et al, 2022. Carga de resultados:
```{r}
load('results/results_INLA_ST_E0c.RData')
```
...
inla.emarginal(fun = function(x) x,marginal = m.ex2$marginals.fixed$intercept)
inla.qmarginal(c(0.025,0.975),marginal = m.ex2$marginals.fixed$intercept)
```
...
inla.emarginal(fun = function(x) x,marginal = outputPC1.field$marginals.range.nominal$range.nominal.1)
inla.qmarginal(c(0.025,0.975),marginal = outputPC1.field$marginals.range.nominal$range.nominal.1)
```
...
inla.emarginal(fun = function(x) x,marginal = outputPC1.field$marginals.variance.nominal$variance.nominal.1)
inla.qmarginal(c(0.025,0.975),marginal = outputPC1.field$marginals.variance.nominal$variance.nominal.1)
```
...
inla.emarginal(fun = function(x) x,marginal = m.ex2$marginals.hyperpar$`GroupRho for idx.PC0`)
inla.qmarginal(c(0.025,0.975),marginal = m.ex2$marginals.hyperpar$`GroupRho for idx.PC0`)
...
inla.emarginal(fun = function(x) 1/x,marginal = m.ex2$marginals.hyper$`Precision for the Gaussian observations`)
1/inla.qmarginal(c(0.025,0.975),marginal = m.ex2$marginals.hyper$`Precision for the Gaussian observations`)
```
...
#ggsave(filename = 'graficoTemps.png',plot = grafico_Temp)
#gsave(filename = 'graficoRango.png',plot = grafico_Rango)
```   
...
write_csv(x = resumenObs_year,file = 'tabla_res_2.csv')
```