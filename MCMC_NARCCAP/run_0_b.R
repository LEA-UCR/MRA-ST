...
#library(gstat)
#vv <- variogram(Y~1,data = hh,cutoff=5000)
#plot(vv)
#### Metropolis Hastings
...
start_beta <- beta
start_A2 <- A2
start_Phi <- rep(3000,nCov_v)
start_tau <- 0.1
start_Nu <- rep(nu,nCov_v)
...
f_P_Nu <- function(beta,A2,Phi,Nu,tau,m2loglike_old,Phi_m,sd_Phi,Nu_m,sd_Nu,status){
  ...
  logpriorphi   <- sum(dunif(Phi,0,5500,log=TRUE)) 
  if(type=='Exponential'){
  ...
  logproposal <- log(dtruncnorm(Phi[1],a = 500,b = 5500,mean = Phi_m[1],sd = sd_Phi[1]))
  #logproposal <- logproposal + log(dtruncnorm(Phi[2],a = 500,b = 3500,mean = Phi_m[2],sd = sd_Phi[2]))
...
proposalfunction_Phi <- function(chain,i,sd_Phi){
  ...
  Yn <- c(rtruncnorm(1,a = 500,b = 3500,mean = mu[1],sd = sd[1]),
          rtruncnorm(1,a = 500,b = 3500,mean = mu[2],sd = sd[2]))
  #Yn <- c(2000,2000)
  return(Yn)
}
...
run_metropolis_MCMC <- function(start_beta,start_A2,start_Phi,start_Nu,start_tau,iterations){
  ...
  for (i in 1:iterations){
    ...
    sd_Phi[1] <- min(sd_Phi[1],1500)
    sd_Phi[2] <- min(sd_Phi[2],1500)
    ...
    show(c(alpha_Phi_Nu,alpha_tau))
    show(c(chain_beta[i+1,],chain_Phi[i+1,],chain_Nu[i+1,],chain_tau[i+1],sd_Nu))
    
    # Prediction
    muhat <- fnew_tau_p$muhat
...
save(MCMC_results, file=paste0("sim_res/chain",variable_narccap,model,type,'NARCCAP_0',".Rdata"))
