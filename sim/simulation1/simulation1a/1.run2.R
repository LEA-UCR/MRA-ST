...
run_metropolis_MCMC <- function(start_beta,#start_A2,
              start_Phi,Sigma,start_tau,iterations,types){
  ...
  for (i in 1:iterations){
    ...
    ## MCMC Phi###
    fold_Phi <- f_P_tau(  chain_beta[i+1,],A1,chain_Phi[i,],Sigma,
  chain_tau[i],m2loglike_old,proposal_Phi,proposal_tau,
                          sd_Phi,sd_tau,'old',types[1],types[2])$like
    fnew_Phi_p <- f_P_tau(chain_beta[i+1,],A1, proposal_Phi,Sigma,
...
chain = run_metropolis_MCMC(start_beta,
        #start_A2,
        start_Phi,Sigma,start_tau,nreps,types)
end_time <- Sys.time()
