...
W_maker <- function(nCov,phi,nu,A2,aa,sigma2, types){ # agregar types
  Tm <- aa[[1]]
  TSm <- aa[[2]]
  Qlist <- aa[[3]]
  nn<- aa[[4]]
...
likelihoodMRA <- function(beta,A2,Phi,Nu,nCov,tau,sigma2,
                          model,nMRA,Y,X,XR,aa,types){
  ...
  for(k in nMRA:0){
    ...
    if(k==nMRA){
      ...
      XSigmae <- XX%*%Sigma_w%*%t(XX)+(tau) * Diagonal(dim(XX)[1])
      ...
      logSigmaYdet <- determinant(XSigmae, logarithm = TRUE)$modulus
    }else{
      ...
      logSigmaYdet <- logSigmaYdet+determinant(Lambda_m[[k+1]]+t(YXB_m) %*% XB_m, logarithm = TRUE)$modulus-
        determinant(Lambda_m[[k+1]], logarithm = TRUE)$modulus
    }
  }
  m2logv <- logSigmaYdet+t(Y-muhat)%*%SigmaYinv%*%(Y-muhat) # -2*loglikelihood
  lista_res <- list(m2logv=m2logv,SigmaYinv=SigmaYinv,
