...
locations <- st_coordinates(hh$geometry)
...
loc.domain = rbind(c(-0.1,-0.1),c(-0.1,1.1),c(1.1,-0.1),c(1.1,1.1))
mesh =
...
A.PC0 = inla.spde.make.A(mesh, loc=locations, 
                         weights = rep(1,N))
...
stk <- inla.stack(data=list(Y=hh$Y), tag='data',
                  A=list(A.PC0,1),
...
save(mod,spde.spatial.vcm,stk,labels, total_time,file=paste0("sim_res/results",
