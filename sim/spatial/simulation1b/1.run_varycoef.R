...
nDEP <- 2   #1="Exponential",2="Matern32",3="Matern52"
type <- DEP[nDEP]
nres <- 1 # or 2
...
hh_sf <- st_as_sf(hh)
locs <- st_coordinates(hh_sf$geometry)
...
# start_A2 <- A2
start_Phi <- c(0.12)
# start_PHI <- seq(0.05,0.15,length.out=10)
# start_Phi <- start_PHI[i]
start_Nu <- ifelse(nDEP== 1, 1/2,4/5)
start_tau <- 0.001
N <- dim(hh)[1] 
# 
# # fixed
types <- c(type)
type.varycoef<-ifelse(type=="Exponential","exp","mat")
#Nu <- rep(3/2, nCov_v)
#A1 <- diag(nCov_v) ## A1
...
proposalfunction_Phi <- function(chain,i,sd_Phi){
  ...
  Yn <- c(rtruncnorm(1,a = 0,b = 1,mean = mu[1],sd = sd[1]))#,
...
proposalfunction_Nu <- function(chain,i,sd_Nu){
  ...
  Yn <- c(rtruncnorm(1,a = 0,b = 2,mean = mu[1],sd = sd[1]))#,
  Yn <- ifelse(nDEP== 1, 1/2,4/5)
  return(Yn)
}
...
run_metropolis_MCMC <- function(start_beta,
                                start_Phi,Sigma,start_Nu,start_tau,iterations,types
                                ,taper=0.1){
  ...
  for (i in 1:iterations){
    ...
    ## MCMC Phi-Nu###
    fold_Phi <- f_P_tau_varycoef(  chain_beta[i+1,],chain_Phi[i,],Sigma,chain_Nu[i],
                                   chain_tau[i],m2loglike_old,proposal_Phi,sd_Phi,proposal_Nu,
                                   sd_Nu,proposal_tau,
                                   sd_tau,'old',type.varycoef[1],taper=taper)$like
    fnew_Phi_p <- f_P_tau_varycoef(chain_beta[i+1,], proposal_Phi,Sigma,proposal_Nu,
...
save(chain,labels, total_time,file=paste0("sim_res/chain",
                                          i,model,taper,type,"_",nres,".Rdata"))
