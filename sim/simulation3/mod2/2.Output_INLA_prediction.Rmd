...
base_grafico_rangos <- bind_rows(data.frame(outputPC0.field$marginals.range.nominal$range.nominal.1,type='PC0'))

ggplot(data = base_grafico_rangos,mapping = aes(x = x,y = y)) + geom_line()+
  theme_bw()+facet_wrap(vars(type),nrow = 2,scales = 'free')

inla.zmarginal(outputPC0.field$marginals.range.nominal[[1]])
```

### Sigma2

```{r}
base_grafico_varianzas <- bind_rows(data.frame(outputPC0.field$marginals.variance.nominal$variance.nominal.1,type='PC0'))

ggplot(data = base_grafico_varianzas,mapping = aes(x = x,y = y)) + geom_line()+
  theme_bw()+facet_wrap(vars(type),nrow = 2,scales = 'free')

inla.zmarginal(outputPC0.field$marginals.variance.nominal[[1]])
```
