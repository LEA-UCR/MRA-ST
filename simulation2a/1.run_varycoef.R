...
# # fixed
types <- c(type)
type.varycoef<-ifelse(DEP=="Exponential","exp","mat32")
#Nu <- rep(3/2, nCov_v)
#A1 <- diag(nCov_v) ## A1
...
proposalfunction_Phi <- function(chain,i,sd_Phi){
  ...
  Yn <- c(rtruncnorm(1,a = 0,b = 5,mean = mu[1],sd = sd[1]))#,
...
proposalfunction_tau <- function(chain,i,sd_tau){
  ...
  tauprop <- rtruncnorm(1,a = 0,b = 3,mean = chain[i],sd = sd_tau)
  #tauprop <- 1
  return(tauprop)
}
...
run_metropolis_MCMC <- function(start_beta,#start_A2,
  ...
  for (i in 1:iterations){
    ...
    ## MCMC Phi###
    fold_Phi <- f_P_tau_varycoef(  chain_beta[i+1,],A1,chain_Phi[i,],Sigma,
                                   chain_tau[i],m2loglike_old,proposal_Phi,proposal_tau,
                                   sd_Phi,sd_tau,'old',type.varycoef[1],taper=taper)$like
    fnew_Phi_p <- f_P_tau_varycoef(chain_beta[i+1,],A1, proposal_Phi,Sigma,
                                   proposal_tau,m2loglike_old,chain_Phi[i,],chain_tau[i],
                                   sd_Phi,sd_tau,'new',type.varycoef[1],taper=taper)
