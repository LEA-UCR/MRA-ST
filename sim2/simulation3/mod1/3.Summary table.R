...
#sigma.real <- 0.0003
tau.real <- 1/700000
parametro.real <- c(beta0.real,phi.real,sigma.real,tau.real)
parametro <- c("beta0","phi","sigma","tau")
...
for(i in 1:10){
  ...
  ##parameter estimation
    scenario<-paste0("sim_res_prediction/results",i,model,type,nVAR,nres,"_prediction.Rdata")
    load(scenario)
    ...
    if(ev_rho){
      rho.est <- inla.zmarginal(mod$marginals.hyperpar$`GroupRho for idx.PC0`)
    }
    ...
    coef.summary <- rbind(betas.est,phi.est,sigma.est,tau.est)
    row.names(coef.summary)[2:4]<-c("phi","sigma","tau")
...
colnames(time.exec) <- paste0(nVAR,nres)

write_csv(coef.summary.list1,file = paste0("output/metrica_IID",nVAR,nres,".csv"))
write_csv(prediction.table,file = paste0("output/metrica_pred_IID",nVAR,nres,".csv"))
write_csv(time.exec, file = paste0("output/time_exec_IID",nVAR,nres,".csv"))
