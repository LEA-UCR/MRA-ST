...
function.to.gen.data <- function(nVAR,nres,i){
  ...
  Sigma <- Sigma[nVAR]
  tau2 <- 700000
  #temporal AR(1) structure parameter
  ...
  ar1_cov <- function(t, rho,sigma) {
    var <- sigma^2/(1-rho^2)
    H <- abs(outer(1:t, 1:t, "-"))
    V <- var * rho^H
    # p <- nrow(V)
    # V[cbind(1:p, 1:p)] <- V[cbind(1:p, 1:p)] * var
    return(V)
  }
  ...
  beta0.cov <- kronecker(ar1_cov(ntime,rho,sigma=0.6),beta0.cov)
  
  # beta1.cov <- rExpMat_regional(n,gridbase_regional,type = type,phi=phi,nu=nu,sigma=Sigma)
  # beta1.cov <- kronecker(ar1_cov(ntime,rho,sigma=1),beta1.cov)
  ...
  Tw <- exp(logTw)
  Ts <- exp(logTs)
  dataset<-tibble(Y=as.vector(Y), Tw=Tw, Ts=Ts,
